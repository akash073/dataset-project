...
SAVED_MODEL_DIR = Path(
    "./moondream_cifar10_finetuned/final_model"
)

CIFAR_ROOT = "./data"

OUTPUT_CSV = Path(
    "./moondream_cifar10_test_predictions.csv"
)

METRICS_JSON = Path(
    "./moondream_cifar10_test_metrics.json"
)

# Set to None to test all 10,000 CIFAR-10 test images.
MAX_TEST_SAMPLES = 10
...
MODEL_NAME = "Moondream2-CIFAR10-Finetuned"
...
print("\nNumber of test images:", number_of_samples)
...
def normalize_prediction(response: str) -> str:
    ...
    for alias, class_name in aliases.items():
        if re.search(
            ...
            alias_matches.append(class_name)
...
def score_candidate_classes(image: Image.Image) -> dict:
    """
    Compute a normalized probability distribution over the 10 CIFAR-10
    class labels.
...
def run_sample_inference(image: Image.Image) -> dict:
    ...
    score the ten valid CIFAR-10 labels for confidence statistics.
    """
...
def run_with_energy_tracking(
    ...
    if CODECARBON_AVAILABLE:
        tracker = EmissionsTracker(
            project_name="moondream_cifar10_test",
            output_dir=str(energy_dir),
            output_file="codecarbon_moondream_cifar10.csv",
            log_level="error",
            save_to_file=True,
        )
...
print("\n" + "=" * 72)
print("TESTING SAVED FINE-TUNED MODEL")
print("=" * 72)
