...
def run_with_energy_tracking(inference_fn, *args, output_dir="./test_results/codecarbon", **kwargs):
    ...
    if CODECARBON_AVAILABLE:
        tracker = EmissionsTracker(
            project_name="cifar10_edge_inference",
...
def _run_torch_model(model, image_tensor):
    """Shared inference path for CIFAR-10 RGB CNN and DNN."""
...
def tensor_to_canvas_array(image_tensor):
    """Convert CIFAR-10 tensor [3,32,32] to uint8 RGB array [32,32,3]."""
    img = image_tensor.detach().cpu().permute(1, 2, 0).numpy() * 255.0
...
def build_row(
    ...
    input_tokens = 3072  # 3x32x32 RGB value proxy
    output_tokens = 1
    ...
    return {
        ...
        "true_class":                   CIFAR10_CLASSES[int(true_label)] if true_label is not None else None,
        "prediction":                   prediction,
        "predicted_class":              CIFAR10_CLASSES[int(prediction)] if prediction is not None and 0 <= int(prediction) < 10 else None,
        "correct":                      correct,
...
def collect_for_model(base_dataset, model_name, num_samples=250, flush_every=25, file_name ='vanilla'):
    ...
    if model_name == "CNN":
        ...
        model.eval()
        model_flops = compute_model_flops(model, DEVICE, input_shape=(1, 3, 32, 32))

    elif model_name == "DNN":
        model = SimpleDNN().to(DEVICE)
        model.load_state_dict(torch.load("cifar10_dnn.pth", map_location=DEVICE))
        model.eval()
        model_flops = compute_model_flops(model, DEVICE, input_shape=(1, 3, 32, 32))
