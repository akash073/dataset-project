...
def run_with_energy_tracking(inference_fn, *args, output_dir="./energy_logs", **kwargs):
    ...
    if CODECARBON_AVAILABLE:
        tracker = EmissionsTracker(
            project_name="classical_model_inference",
            output_dir=output_dir,
            output_file="codecarbon_classical_models.csv",
            log_level="error",
            save_to_file=True,
        )
        tracker.start()
...
#     # Match the quantum experiment naming/layout pattern:
#     # <device_short_id>/classical_model_results.csv
#     device_log_dir = ROOT / f"{DEVICE_SHORT}"
#     device_log_dir.mkdir(parents=True, exist_ok=True)
#     return device_log_dir / "classical_model_results.csv"
...
def get_edge_dataset_path():
    cpu_name = safe_filename(CPU_MODEL_NAME)
    ram_gb = int(round(SYSTEM_RAM_TOTAL_GB))
    cpu_cores = CPU_CORE_COUNT
    device_log_dir = ROOT / "logs" /f"{DEVICE_SHORT}"
    device_log_dir.mkdir(parents=True, exist_ok=True)
    return device_log_dir / (
        f"classical_results_{cpu_name}_{cpu_cores}cores_{ram_gb}GB_RAM.csv"
    )
    ...
    try:
        ...
        if "collection_mode" in df.columns:
            mask = (
                ...
                & (df["collection_mode"].astype(str).str.strip() == "classical_model_codecarbon")
            )
            return int(mask.sum())
...
def build_row(
    ...
    return {
        ...
        "collection_mode":              "classical_model_codecarbon",
        ...
        "parameter_count":              model_metrics.get("parameter_count", model_metrics.get("parameters")),
        "model_flops":                  model_flops,

        # --- Prediction quality ---
        "confidence_score":             confidence_score,
        "score_margin":                 logit_margin,
        "entropy":                      entropy,
